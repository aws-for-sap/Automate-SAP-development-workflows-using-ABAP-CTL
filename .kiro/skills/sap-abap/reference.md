...
- `odata list`: live V2+V4 catalog. Per-catalog failure → warning (not fatal); both 403/404 → `CAPABILITY_NOT_AVAILABLE` (EC6).
...
- `table` (TABL/DT, capability-gated on EC6), `structure` (TABL/DS, works on EC6)
...
| `bdef listinterfaces <name>` | none | read-only (assigned BO interfaces; usually empty; 404 → `[]` on EC6) |
